...
export function renderReports(root) {
  if (!can('reports.view')) {
    root.innerHTML = `
      <div class="flex flex-col items-center justify-center h-64 text-center">
        ...
        <h2 class="text-xl font-semibold text-brand-darkest">Acceso restringido</h2>
        <p class="text-sm text-brand-base mt-1">No tienes permisos para ver esta sección.</p>
      </div>`;
    return;
  }
  ...
  root.innerHTML = `
    <h1 class="text-2xl font-bold text-brand-darkest mb-6">Reportes</h1>

    <!-- KPIs -->
    <div class="grid grid-cols-3 gap-4 mb-8">
      ${kpi('Valor total inventario', `$${valorTotal.toFixed(2)}`, 'bg-brand-base')}
      ${kpi('Unidades ingresadas', totalEntradas, 'bg-brand-base')}
      ${kpi('Unidades egresadas', totalSalidas, 'bg-brand-dark')}
    </div>

    <!-- Por categoría -->
    <div class="bg-white rounded-xl shadow p-4 mb-6">
      <h2 class="font-semibold text-brand-darkest mb-3">Inventario por categoría</h2>
      ${Object.keys(byCategory).length
        ? `<table class="w-full text-sm">
            <thead class="text-brand-dark text-xs uppercase">
              <tr>
        ...
        : `<p class="text-brand-base text-sm">Sin datos.</p>`}
    </div>

    <!-- Stock bajo -->
    <div class="bg-white rounded-xl shadow p-4">
      <h2 class="font-semibold text-brand-darkest mb-3">⚠️ Productos con stock bajo</h2>
      ${lowStock.length
        ? `<ul class="space-y-1 text-sm">
            ${lowStock.map(p => `
              <li class="flex justify-between border-b py-1">
                <span>${p.name}</span>
                <span class="text-brand-dark font-medium">Stock: ${p.stock} / Mín: ${p.minStock}</span>
              </li>`).join('')}
          </ul>`
        : `<p class="text-brand-base text-sm">✅ Todos los productos tienen stock suficiente.</p>`}
    </div>`;
}
